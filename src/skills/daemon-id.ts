...
const PATH_CACHE = join(homedir(), ".local", "share", "operad", "daemon-id.path");
...
export function resolveDaemonId(): DaemonIdResult {
  ...
  mkdirSync(dirname(PATH_CACHE), { recursive: true, mode: 0o700 });
  writeFileSync(PATH_CACHE, chosen, { mode: 0o600 });

  return { id, path: chosen, may_sync: isSyncProneFallback(chosen) };
}
 ...
 */
export function chooseLocation(): string {
  ...
  return join(homedir(), ".local", "share", "operad", "daemon-id");
}
...
function isSyncProneFallback(p: string): boolean {
  return p.startsWith(join(homedir(), ".local", "share"));
}
