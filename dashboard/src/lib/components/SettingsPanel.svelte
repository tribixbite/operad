...
<div class="settings-root">
  ...
  {:else if data}
    ...
    <!-- Section 3.5: Plans -->
    <div class="card section-card">
      <div class="section-header" role="button" tabindex="0" onclick={() => toggleSection("plans")}>
        <span class="chevron">{sections.plans ? "▾" : "▸"}</span>
        <span class="section-title">Plans</span>
        <span class="badge badge-blue">{(data.plans ?? []).length}</span>
        <span class="header-actions" onclick={stopProp}>
          <button
            class="btn-icon btn-sm-icon"
            title="Download all as zip"
            disabled={downloadingAll === "plans"}
            onclick={() => handleDownloadAll(data!.plans ?? [], "plans")}
          >{downloadingAll === "plans" ? "..." : "\u2913"}</button>
        </span>
      </div>
      {#if sections.plans}
        <div class="section-body">
          {#if (data.plans ?? []).length === 0}
            <p class="muted">No plan files found</p>
          {:else}
            <div class="item-list">
              {#each data.plans ?? [] as plan (plan.path)}
                <div class="item-row">
                  <button class="item-header" onclick={() => toggleExpand(plan.path)}>
                    <span class="chevron">{expandedItem === plan.path ? "▾" : "▸"}</span>
                    <span class="item-name">{plan.name}</span>
                    <span class="badge" class:badge-blue={plan.scope === "user"} class:badge-dim={plan.scope === "project"}>
                      {plan.scope}
                    </span>
                    <span class="item-path" title={plan.path}>{shortenPath(plan.path)}</span>
                  </button>
                  {#if expandedItem === plan.path}
                    <div class="item-content">
                      {#if loadingFile === plan.path}
                        <p class="muted">Loading...</p>
                      {:else if editingItem === plan.path}
                        <textarea
                          class="edit-area"
                          bind:value={editBuffer}
                          rows="20"
                        ></textarea>
                        <div class="edit-actions">
                          <button class="btn btn-primary btn-sm" onclick={() => saveEdit(plan.path)} disabled={savingFile}>
                            {savingFile ? "Saving..." : "Save"}
                          </button>
                          <button class="btn btn-sm" onclick={cancelEdit}>Cancel</button>
                        </div>
                      {:else}
                        <div class="file-actions">
                          <button class="btn btn-sm" onclick={() => startEdit(plan.path)}>Edit</button>
                          <button class="btn btn-sm" onclick={() => handleDownload(plan.path)}>Download</button>
                          <button class="btn btn-sm" onclick={() => handleShare(plan.path)}>Share</button>
                        </div>
                        <pre class="file-preview">{fileContents[plan.path] ?? ""}</pre>
                      {/if}
                    </div>
                  {/if}
                </div>
    ...
    <!-- Section 6b: Commands (.claude/commands/*.md) -->
    <div class="card section-card">
      <button class="section-header" onclick={() => toggleSection("commands")}>
        <span class="chevron">{sections.commands ? "▾" : "▸"}</span>
        <span class="section-title">Commands</span>
        <span class="badge badge-blue">{(data.commands ?? []).length}</span>
      </button>
    ...
    <!-- Section 6c: Subagents (.claude/agents/*.md) -->
    <div class="card section-card">
      <button class="section-header" onclick={() => toggleSection("subagents")}>
        <span class="chevron">{sections.subagents ? "▾" : "▸"}</span>
        <span class="section-title">Subagents (.claude/agents/)</span>
        <span class="badge badge-blue">{(data.agentsMd ?? []).length}</span>
      </button>
    ...
    <!-- Section 6d: Memories (.claude/memories/*.md) -->
    <div class="card section-card">
      <button class="section-header" onclick={() => toggleSection("memories")}>
        <span class="chevron">{sections.memories ? "▾" : "▸"}</span>
        <span class="section-title">Memories (.claude/memories/)</span>
        <span class="badge badge-blue">{(data.memories ?? []).length}</span>
      </button>
    ...
    <!-- Section 6e: AGENTS.md (cross-tool compat) -->
    <div class="card section-card">
      <button class="section-header" onclick={() => toggleSection("agentsMd")}>
        <span class="chevron">{sections.agentsMd ? "▾" : "▸"}</span>
        <span class="section-title">AGENTS.md</span>
        <span class="badge badge-blue">{(data.agentsMdFiles ?? []).length}</span>
      </button>
      {#if sections.agentsMd}
        <div class="section-body">
          <AgentsMdPanel
            agentsMdFiles={data.agentsMdFiles ?? []}
            selectedProject={selectedProject}
            projectName={selectedProject ? (selectedProject.split("/").pop() ?? "") : ""}
            onExpand={toggleExpand}
          />
