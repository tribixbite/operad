          ...
          <tbody>
            {#each userAgents as agent}
              <tr>
                ...
                <td class="muted small meta-col">{formatRelativeTime(agent.modified)}</td>
                <td class="muted small meta-col mono">{formatBytes(agent.size)}</td>
                <td class="path-col">
                  <button class="path-icon-btn" title={agent.path} onclick={() => copyToClipboard(agent.path)} aria-label="Copy path to clipboard">
